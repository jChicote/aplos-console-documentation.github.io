...
| `ApplySettingsTo(GameObject root, bool includeChildren = true)` | Applies the persisted settings to a freshly spawned object (e.g. a runtime window) so it matches the saved configuration immediately, without waiting for the next full scene rescan. |
